...
app.get("/api/v1/notes", function(req, res) {
    ...
    const dbFile = fs.readFileSync(path.join(__dirname,  "db.json"), "utf8");
...
app.post("/api/v1/notes", function(req, res) {
    ...
    let dbFile = fs.readFileSync(path.join(__dirname, "db.json"), "utf8");
    ...
    fs.writeFileSync(path.join(__dirname,  "db.json"), JSON.stringify(dbFileJSON), "utf8");
...
app.delete("/api/v1/notes/:id", function(req, res) {
    ...
    let newDBFile = dbFileJSON.filter((note) => note.id != noteId);n
    console.log(JSON.stringify(newDBFile));
    //write the new dbFile
    fs.writeFileSync(path.join(__dirname,  "db.json"), JSON.stringify(newDBFile), "utf8");
